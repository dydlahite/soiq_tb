...
from config import OPENAI_API_KEY, PROXYAPI_API_KEY, PROXYAPI_BASE_URL, PROXYAPI_TTS_MODEL, PROXYAPI_TTS_VOICE
...
def get_voice_chance():
    ensure_voice_defaults()
    return get_int_setting("voice_chance", 10, 0, 100)
...
def can_send_voice():
    return get_voice_count_today() < get_voice_max_per_day() and voice_cooldown_passed()


def user_asks_voice(user_text):
    text_l = (user_text or "").lower()
    phrases = ["ответь голосом", "скинь голосовое", "запиши гс", "можешь голосом", "голосом ответь", "гс"]
