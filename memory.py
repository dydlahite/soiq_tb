...
def list_memories(scope, user_id=None, chat_id=None):
    if scope == "global":
        cursor.execute("SELECT key, value FROM memories WHERE scope = 'global' ORDER BY key")
    else:
        cursor.execute(
            "SELECT key, value FROM memories WHERE scope = ? AND user_id = ? AND chat_id = ? ORDER BY key",
