...
def choose_idle_thought():
    return random.choice(load_idle_thoughts())
...
async def idle_check_job(context):
    ...
    for target in targets:
        ...
        try:
            ...
            save_message(user_id, chat_id, "assistant", thought)
...
async def send_idle_now(bot, chat_id):
    ...
    cursor.execute("SELECT user_id FROM chats WHERE chat_id = ? LIMIT 1", (chat_id,))
    row = cursor.fetchone()
    if row and row["user_id"]:
        save_message(row["user_id"], chat_id, "assistant", thought)
    mark_idle_sent(chat_id)
    return thought
