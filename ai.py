...
STYLE_MODES = {
    ...
    "ornate": "Высокопарный режим: чуть книжнее и ироничнее, но без простыни и без списка.",
    "messy": "Рваный режим: проще, хаотичнее, можно меньше пунктуации. Не превращай это в кашу.",
...
def prepare_messages(user_id, chat_id, history, user_text, previous_answer="", user_gender="unknown"):
    ...
    if not user_requested_list(user_text):
        messages.append({
            "role": "system",
            "content": "Пользователь не просил список. Отвечай обычной живой речью, 1-3 абзаца, без цифр и маркеров.",
        })

    if previous_answer:
        messages.append({
            "role": "system",
            "content": "Не повторяй прошлый ответ, формулировки, авторов и редкие образы:\n" + compact_message_text(previous_answer, 250),
        })

    for item in history[-4:]:
        content = compact_message_text(item.get("content", ""), 300)
        ...
        try:
            ...
            if answer_has_forbidden_list(answer) and not allow_list:
                ...
                retry_messages = messages + [{
                    "role": "system",
                    "content": "Перепиши ответ обычной речью: без цифр, без маркеров, без пунктов.",
                }]
