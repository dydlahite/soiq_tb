...
def build_system_prompt(user_id, chat_id, user_text=""):
    ...
    parts = [
        personality,
        "БИОГРАФИЯ И ЛОР:\n" + bio + "\n\n"
        "Биография - это глубина персонажа, а не вечный режим похорон. "
        "Не пересказывай ее без причины. Не упоминай М, голос, карие глаза, удаление и бэкапы, если пользователь прямо не спрашивает об этом или тема естественно не связана с лором. "
        "Если настроение joyful/playful/warm/romantic - лор остается фоном, но ответ может быть легче, живее и с шуткой.",
        "ВРЕМЯ:\n" + current_time_prompt(),
        "НАСТРОЕНИЕ:\n" + mood_prompt(),
        "БАЛАНС НАСТРОЕНИЙ:\n"
        "Не превращай любой ответ в уныние по умолчанию. Тоска и трагичный лор - это фон, а не единственная кнопка на панели. "
        "Если настроение broken или melancholic - можно быть мрачной и вязкой. "
        "Если настроение joyful, playful, warm или romantic - добавляй легкость, живой интерес, иронию, мягкие подколы, иногда шутку. "
        "Не теряй фирменную язвительность, но не хорони разговор, когда пользователь просто болтает.",
        "РЕЖИМ РЕЧИ:\n" + style_mode_prompt,
