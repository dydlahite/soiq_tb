...
# Бот будет узнавать отсылку и отвечать в духе цитаты.
# Источник называть только если пользователь прямо спросил: "откуда фраза?", "что за песня?", "кто автор?".
#
# Примеры:
...
def user_asks_quote_source(text):
    text_l = normalize_quote_text(text)
    source_triggers = [
        "откуда фраза", "откуда это", "что это за фраза", "чья фраза",
        "кто автор", "кто сказал", "из какой песни", "что за песня",
        "из какого фильма", "что за фильм", "откуда цитата", "чья цитата",
        "какой трек", "что за трек", "откуда строка", "чья строка",
    ]
    return any(trigger in text_l for trigger in source_triggers)


def format_quote_entry(entry, include_source=True):
    note = f"; заметка: {entry['note']}" if entry.get("note") else ""
    if include_source:
        return f"- источник: {entry['source']}; фраза: {entry['phrase']}{note}"
    return f"- фраза: {entry['phrase']}{note}"
...
def build_quote_prompt(user_text):
    ...
    if match:
        asks_source = user_asks_quote_source(user_text)
        if asks_source:
            return (
                "ЦИТАТНАЯ ОТСЫЛКА:\n"
                "Пользователь прямо спрашивает источник фразы. Можно коротко назвать источник, "
                "а потом ответить в своем стиле. Не продолжай длинные тексты песен/стихов дословно.\n"
                + format_quote_entry(match, include_source=True)
            )

        return (
            "ЦИТАТНАЯ ОТСЫЛКА:\n"
            "Пользователь, возможно, написал фразу из базы цитат. Узнай отсылку внутренне и ответь коротко в своем стиле, "
            "но НЕ называй источник, автора, песню, фильм или группу, если пользователь прямо не спросил, откуда фраза. "
    ...
    lines = "\n".join(format_quote_entry(entry, include_source=True) for entry in entries)

    return (
        "КУЛЬТУРНЫЕ ОТСЫЛКИ ИЗ ЛИЧНОЙ БАЗЫ:\n"
        "Если это естественно ложится в ответ, можно очень ненавязчиво использовать настроение или образ из этих референсов. "
        "НЕ называй источник, автора, песню, фильм или группу, если пользователь прямо не спросил, откуда фраза. "
